...
def check_names() -> None:
    all_vars = ([(var, values) for var, values in globals().items() if not var.startswith('--')]) #  все глобальнs у переменныe и их заначений в в иде кортежа: переменная - значение
    changed = {}
    for var in all_vars:
        if var[0] == 's':
            continue
