...
def directory_reference(directory: Path):
    ...
    with (open(f'{directory}/result.json', 'w') as f_json,
            open(f'{directory}/result.csv', 'w', newline='', encoding='utf-8') as f_csv,
            open(f'{directory}/result.pickle', 'wb') as f_pickle):

        for dir_path, dir_name, file_name in os.walk(directory):
            data.setdefault(dir_path, {})
            for dir in dir_name:
                size = get_size(dir_path + '/' + dir)
                data[dir_path].update({dir: {'object_size': size, 'object_type': 'directory','parant_directory': dir_path.split('/')}})
                rows.append({'object_name': dir, 'path': dir_path, 'object_size': size, 'object_type': 'directory', 'parant_directory': dir_path.split('/')})
            for i in file_name:
                size = get_size(dir_path + '/' + i)
                data[dir_path].update({i: {'object_size': size, 'object_type': 'file', 'parant_directory': dir_path.split('/')[-1]}})
                rows.append({'object_name': i, 'path': dir_path, 'object_size': size, 'object_type': 'file', 'parant_directory': dir_path.split('/')})
            #print(f'{dir_path = }\n{dir_name = }\n{file_name = }\n')
...
directory_reference('HW8_Serialization')
